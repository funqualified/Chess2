const Title = () => {
  ...
  var gameTitle = "";
  if (Math.random() < 0.5) {
    gameTitle = randomGameNames[Math.floor(Math.random() * randomGameNames.length)];
    if (gameTitle.includes("[insert piece name]")) {
      var pieceName = randomPieceNames[Math.floor(Math.random() * randomPieceNames.length)];
      gameTitle = gameTitle.replace("[insert piece name]", pieceName);
    }
  } else {
    gameTitle = "Chess 2: Game of the Year Edition";
  }

  var classList = "";
  if (Math.random() < 0.01) {
    classList += "bounce-title ";
  }
  if (Math.random() < 0.05) {
    classList += "shake-title ";
  }
  if (Math.random() < 0.33) {
    classList += "shine-title ";
  }
  if (Math.random() < 0.05) {
    classList += "tiny-title ";
  }
  if (Math.random() < 0.1) {
    classList += "red-title ";
  }
